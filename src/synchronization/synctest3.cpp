...
// synctest*.cpp. In synctest3.cpp, blocking communication is used.
// A time consuming task is assigned to one of the cores to
// demonstrate the impact of blocking/non-blocking communications.
// The impact of communication tags is tested in synctest3.cpp.
//
// Compile: mpic++ -O2 synctest3.cpp -o synctest3.exe
//
// Run: mpirun -n <number of cores> synctest3.exe
//
...
int main(int argc, char **argv)
{
	...
	for(task=1;task<=numoftasks;task++){
		...
		if(core == mympi.nID && mympi.nID!=0)
		{
			...
			if(core == 2)
				findPrime(10000);
			MPI_Send(&task,1,MPI_INT,0,(10+core),MPI_COMM_WORLD);
		}
		//---------------------------------------------------------
		...
		if (mympi.nID == 0){
			if (core == 0)
				...
				MPI_Recv(&ctask,1,MPI_INT,MPI_ANY_SOURCE,(10+core),MPI_COMM_WORLD,&status);
